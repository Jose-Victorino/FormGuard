...
function Profile() {
  ...
  const [draftName, setDraftName] = useState({ first_name: '', last_name: '' })
  ...
  const handleEditName = () => {
    setDraftName({ first_name: firstName, last_name: lastName })
    setEditName(true)
  }
  ...
  const handleNameChange = (field) => (e) => {
    ...
    setDraftName((prev) => ({ ...prev, [field]: value }))
  }
  ...
  const handleSaveName = () => {
    if (!userId) return
    const payload = {
      first_name: wordCap(draftName.first_name),
      last_name: wordCap(draftName.last_name),
    }
    patchUser(payload)
          ...
          {editName ?
            <div className='flex-col gap-15'>
              ...
              <div className='flex gap-10'>
                <Button text='Save' color='green' size='sm' onClick={handleSaveName} />
                <Button text='Cancel' color='light' size='sm' onClick={handleCancelName} />
              </div>
            </div> :
            <div className='flex gap-15 a-center'>
              <p>{`${firstName} ${lastName}`}</p>
              <Button text='Edit' color='light' size='sm' onClick={handleEditName} />
            </div>
          }
        </div>
        <div className='flex-col gap-5'>
